...
Polygon simplify_KiMPA(Polygon input){
    ...
    return Simplification::Polygon_Algo_KiMPA(output, output.size()/4);
...
int main(int argc,char *argv[])
{
    if(argc < 3){
        ...
        std::cerr << "Usage : comparison_stats DIRECTORY OUT_FILE" << std::endl;
        return 1;
    }
    ...
    // Searching classes
    std::cout << "Searching classes..."<< std::endl;
    std::vector<std::string> classes;
    for (int i = 0; i < fileList.count(); i++){
        std::string c = (QString::fromStdString(in)+"/"+fileList[i]).toStdString();
        c = getClass(c);
    ...
    std::cout << std::endl;
    file << std::endl;
    std::cout << "classes found : " <<  classes.size() << std::endl;
	...
	std::vector<ComparedTF> listPoly;
    ...
    for (int i = 0; i < fileList.count(); i++){
		...
		current.class_ = QString::fromStdString(getClass(file.toStdString()));
        current.num = i;

		listPoly.push_back(current);

	}    
    std::cout << std::endl;
	std::cout << "Loading files and simplifying shapes finished"<< std::endl;


	// Comparing
	std::cout << "Start comparison..."<< std::endl;
    for (int i = 5; i < classes.size(); i++){
        std::cout << classes.at(i) << std::endl;
        write_stats(QString::fromStdString(classes.at(i)), listPoly, classes, file);
    }
